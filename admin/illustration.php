<?php
    ...
    if(isset($_GET['delete']))
    {
        ...
        $verif = $bdd->prepare("SELECT * FROM illustration WHERE id=?");
        $verif->execute([$id]);
        ...
        $delete->execute([$id]);
        $delete->closeCursor();
        header("LOCATION:illustration.php?successDelete=".$id);
...
<body>
    ...
    <div class="container">
        ...
        <div>
            <a href="addIllustration.php" class="btn btn-primary my-3">Ajouter une illustration</a>
        </div>
        <?php
                    ...
                    while($don = $req->fetch())
                    {
                        echo "<tr>";
                            ...
                            echo "<td class='text-center'>";
                                echo "<a href='updateIllustration.php?id=".$don['id']."' class='btn btn-warning m-2'>Modifier</a>";
                                echo "<a href='illustration.php?delete=".$don['id']."' class='btn btn-danger m-2'>Supprimer</a>";
                            echo "</td>";
                        echo "</tr>";
                    }
                    $req->closeCursor();
