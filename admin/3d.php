<?php
    ...
    if(isset($_GET['delete']))
    {
        ...
        $verif = $bdd->prepare("SELECT * FROM 3d WHERE id=?");
        $verif->execute([$id]);
        ...
        $delete = $bdd->prepare("DELETE FROM 3d WHERE id=?");
        $delete->execute([$id]);
        $delete->closeCursor();
        header("LOCATION:3d.php?successDelete=".$id);
...
<body>
    ...
    <div class="container">
        ...
        <div>
            <a href="add3d.php" class="btn btn-primary my-3">Ajouter un élément 3D</a>
        </div>
        <?php
                    ...
                    while($don = $req->fetch())
                    {
                        echo "<tr>";
                            ...
                            echo "<td class='text-center'>";
                                echo "<a href='update3d.php?id=".$don['id']."' class='btn btn-warning m-2'>Modifier</a>";
                                echo "<a href='3d.php?delete=".$don['id']."' class='btn btn-danger m-2'>Supprimer</a>";
                            echo "</td>";
                        echo "</tr>";
                    }
                    $req->closeCursor();
