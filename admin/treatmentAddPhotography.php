        ...
        if($err==0){
            ...
            if(!isset($erreur))
            {
                ...
                if(move_uploaded_file($_FILES['image']['tmp_name'], $dossier.$fichiercptl))
                {
                    ...
                    $insert = $bdd->prepare("INSERT INTO photography(title,date,description,cover) VALUES(?,?,?,?)");
                    $insert->execute([$title,$date,$description,$fichiercptl]);
                    $insert->closeCursor();
                    header("LOCATION:photography.php?addsuccess=ok");
                }else{
                    header("LOCATION:addPhotography.php?errorimg=3");
                }             
            }else{
                header("LOCATION:addPhotography.php?errorimg=".$erreur);
            }


        }else{
            header("LOCATION:addPhotography.php?error=".$err);
        }
