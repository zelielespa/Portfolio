        ...
        if($err==0){
            ...
            if(!isset($erreur))
            {
                ...
                if(move_uploaded_file($_FILES['image']['tmp_name'], $dossier.$fichiercptl))
                {
                    ...
                    $insert = $bdd->prepare("INSERT INTO compositing(title,date,description,cover) VALUES(?,?,?,?)");
                    $insert->execute([$title,$date,$description,$fichiercptl]);
                    $insert->closeCursor();
                    header("LOCATION:compositing.php?addsuccess=ok");
                }else{
                    header("LOCATION:addCompositing.php?errorimg=3");
                }             
            }else{
                header("LOCATION:addCompositing.php?errorimg=".$erreur);
            }


        }else{
            header("LOCATION:addCompositing.php?error=".$err);
        }
