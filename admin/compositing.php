<?php
    ...
    if(isset($_GET['delete']))
    {
        ...
        $verif = $bdd->prepare("SELECT * FROM compositing WHERE id=?");
        $verif->execute([$id]);
        ...
        $delete = $bdd->prepare("DELETE FROM compositing WHERE id=?");
        $delete->execute([$id]);
        $delete->closeCursor();
        header("LOCATION:compositing.php?successDelete=".$id);
...
<body>
    ...
    <div class="container">
        ...
        <div>
            <a href="addCompositing.php" class="btn btn-primary my-3">Ajouter un compositing</a>
        </div>
        <?php
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <?php
                    $req = $bdd->query("SELECT * FROM compositing");
                    while($don = $req->fetch())
                    {
                        echo "<tr>";
                            echo "<td>".$don['id']."</td>";
                            ...
                            echo "<td class='text-center'>";
                                echo "<a href='updateCompositing.php?id=".$don['id']."' class='btn btn-warning m-2'>Modifier</a>";
                                echo "<a href='compositing.php?delete=".$don['id']."' class='btn btn-danger m-2'>Supprimer</a>";
                            echo "</td>";
                        echo "</tr>";
                    }
                    $req->closeCursor();
