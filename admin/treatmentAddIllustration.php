        ...
        if($err==0){
            ...
            if(!isset($erreur))
            {
                ...
                if(move_uploaded_file($_FILES['image']['tmp_name'], $dossier.$fichiercptl))
                {
                    ...
                    $insert = $bdd->prepare("INSERT INTO illustration(title,date,description,cover) VALUES(?,?,?,?)");
                    $insert->execute([$title,$date,$description,$fichiercptl]);
                    $insert->closeCursor();
                    header("LOCATION:illustration.php?addsuccess=ok");
                }else{
                    header("LOCATION:addIllustration.php?errorimg=3");
                }             
            }else{
                header("LOCATION:addIllustration.php?errorimg=".$erreur);
            }


        }else{
            header("LOCATION:addIllustration.php?error=".$err);
        }
