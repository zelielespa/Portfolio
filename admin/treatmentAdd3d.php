        ...
        if($err==0){
            ...
            if(!isset($erreur))
            {
                ...
                if(move_uploaded_file($_FILES['image']['tmp_name'], $dossier.$fichiercptl))
                {
                    ...
                    $insert = $bdd->prepare("INSERT INTO 3d(title,date,description,cover) VALUES(?,?,?,?)");
                    $insert->execute([$title,$date,$description,$fichiercptl]);
                    $insert->closeCursor();
                    header("LOCATION:3d.php?addsuccess=ok");
                }else{
                    header("LOCATION:add3d.php?errorimg=3");
                }             
            }else{
                header("LOCATION:add3d.php?errorimg=".$erreur);
            }


        }else{
            header("LOCATION:add3d.php?error=".$err);
        }
